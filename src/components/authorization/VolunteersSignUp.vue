  ...
  <div slot="volunteers">
    ...
    <v-text-field v-model="userData.email" placeholder="введите Ваш Email" 
		...
		<v-select name="primary skill" v-model="userData.primarySkill" 
			:items="options.skills" dense solo>
		</v-select>
		...
		<v-select name="equipment" v-model="userData.equipment" 
		:items="options.equipment" dense solo>
			</v-select>
		<label class="text-body-1" for="prefered">Предпочтительный способ связи</label>
		<v-checkbox v-for="contact in options.contacts" :key="contact"	
		...
		mounted() {
			...
			function setOptions (options) {
				...
				for(let option in parsedOptions) {
					selectListService.getOptions(option)
						.then(result => options[option] = result)
				};
			};
